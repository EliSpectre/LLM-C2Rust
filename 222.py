...
def read_cpp_files(filenames):
    """读取多个C语言文件内容，并在每个文件前加上序号标识"""
    contents = []
    for i, filename in enumerate(filenames, start=1):
        try:
            with open(filename, 'r', encoding='utf-8') as file:
                content = file.read()
                contents.append(f"=== 这是第 {i} 个文件: {filename} ===\n{content}\n")
        except Exception as e:
            contents.append(f"=== 这是第 {i} 个文件: {filename} ===\n读取文件出错: {str(e)}\n")
    return "\n".join(contents)

# 获取C语言代码
cpp_files = ["main.c", "scheduler.c","scheduler.h"]  # 替换为你的文件名
cpp_code = read_cpp_files(cpp_files)

if cpp_code.startswith("读取文件出错"):
    print(cpp_code)
    exit(1)
...
里边有多个C语言的文件，第一个是主文件main.c，其他C语言文件更多是一些函数，注意同时也要改进，将其都转换为Rust语言，每个文件的开始有一个index，代表其是第几个文件
